...
# ---- ensure central-nginx is on the project's network (idempotent) ----
NETWORK_NAME="${PROJECT_NAME}_app-network"
if docker network ls --format '{{.Name}}' | grep -qx "$NETWORK_NAME"; then
  docker network connect "$NETWORK_NAME" central-nginx 2>/dev/null || true
fi

# ---- wait until the project's containers are RUNNING, then reload nginx ----
FE_C="${PROJECT_NAME}-frontend-1"
BE_C="${PROJECT_NAME}-backend-1"
POLL_INTERVAL=2            # seconds
TIMEOUT=180                # total seconds to wait
deadline=$((SECONDS + TIMEOUT))

echo "Waiting for containers to be RUNNING: $FE_C, $BE_C"
while (( SECONDS < deadline )); do
  fe_running=$(docker inspect -f '{{.State.Running}}' "$FE_C" 2>/dev/null || echo false)
  be_running=$(docker inspect -f '{{.State.Running}}' "$BE_C" 2>/dev/null || echo false)
  if [[ "$fe_running" == true && "$be_running" == true ]]; then
    break
  fi
  sleep "$POLL_INTERVAL"
done

# Optional: warn if timeout hit
if (( SECONDS >= deadline )); then
  echo "WARN: Containers not RUNNING within ${TIMEOUT}s; continuing to DNS checks…"
fi

# ---- ensure Docker DNS inside central-nginx can resolve their names ----
wait_dns_in_central() {
  local host="$1"; local timeout="${2:-60}"; local start=$SECONDS
  while (( SECONDS - start < timeout )); do
    if docker exec central-nginx getent hosts "$host" >/dev/null 2>&1; then
      return 0
    fi
    sleep 1
  done
...
for H in "$FE_C" "$BE_C"; do
  echo "Waiting for central-nginx to resolve $H…"
  wait_dns_in_central "$H" 60 || echo "WARN: $H not resolvable yet; reload will still proceed."
done

# ---- reload nginx ONCE (validate first) ----
echo "Reloading central-nginx config…"
docker exec central-nginx nginx -t && docker exec central-nginx nginx -s reload || docker restart central-nginx
...
echo
echo "== Storage checks =="
sudo du -sh "${PG_DIR}" || true
sudo df -h "${PG_DIR}" || true
...
if docker ps --format '{{.Names}}' | grep -q "^${PROJECT_NAME}-postgres-1$"; then
  docker exec "${PROJECT_NAME}-postgres-1" df -h /var/lib/postgresql/data || true
else
